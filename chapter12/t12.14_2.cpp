...
//核心处理函数
shared_ptr<connection> f(destination &);
...
int main()
{
    ...
    print(dest1);
    //更接近真实场景的应用，拿到connection用完后不用手动关闭
    auto ptr = f(dest1);
    print(*ptr);
    cout << "main即将结束" << endl;
    return 0;
...
connection *connect(destination *destP)
{
    ...
    connection *conn = new connection({destP->id + 1, "2020-02-23 16:10:01", "连接成功", *destP});
    return conn;
}
...
shared_ptr<connection> f(destination &d)
{
