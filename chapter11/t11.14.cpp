...
{
    //保存名和生日
    map<string, vector<pair<string, string>>> mmap;
    string familyName;
    string firstName;
    string birthDay;
    while (true)
    {
        ...
        while (cin >> firstName)
        {
            ...
            cout << "请输入他（她）的生日" << endl;
            cin >> birthDay;
            pair<string, string> tempPair = {firstName, birthDay};
            mmap[familyName].push_back(tempPair);
        }
    ...
    for (const auto &item : mmap)
    {
        ...
        cout << "分别是：";
        for (const auto &tempPair : item.second)
        {
            //每个tempPair是pair<string, string>
